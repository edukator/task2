...
%   plot total-variation distances is kept.
%
%   Name/value pairs:
...
%   together with metadata such as the number of observations.
    ...
    X0A = x0 - 1 + 1/2*randn([Dx N]);
    X0B = x0 + 1 + 2*randn([Dx N]);
    ...
    sir_barrier(F, sx, sz, he, NTe, n_obs, ze_sparse, H, X0A, ness_thr, barrier_params, optsA);
    ...
    sir_barrier(F, sx, sz, he, NTe, n_obs, ze_sparse, H, X0B, ness_thr, barrier_params, optsB);
    ...
    for obs_idx = 1:num_obs
        ...
        tv_inside(obs_idx) = 0.5 * sum(abs(masses_A - masses_B));
    end

    tv_summary = struct('obs_indices', obs_indices, 'inside_only', tv_inside);
